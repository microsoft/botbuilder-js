...
const generateParseTest = (input, expectedOutput, expectedRefs) => async () => {
    const parsed = Expression.parse(input.toString());
    assert(parsed != null);

    const { value: actualOutput, error } = await parsed.tryEvaluate(scope);
    assert(error == null);
    ...
    const { value: newOutput } = await Expression.parse(parsed.toString()).tryEvaluate(scope);
    assertObjectEquals(actualOutput, newOutput, input);
};

describe('expression parser functional test', async function () {
    ...
    describe('StackedMemory', function () {
        it('supports null values', async function () {
            ...
            assert.deepStrictEqual(await Expression.parse('d').tryEvaluate(sm), { error: undefined, value: 'd' }, 'd');
            assert.deepStrictEqual(await Expression.parse('a').tryEvaluate(sm), { error: undefined, value: 'newa' }, 'a');
            assert.deepStrictEqual(await Expression.parse('c').tryEvaluate(sm), { error: undefined, value: 'c' }, 'c');
            assert.deepStrictEqual(await Expression.parse('b').tryEvaluate(sm), { error: undefined, value: null }, 'b');
        });
    });
    ...
    it('Test AccumulatePath', async function () {
        const scope = {
            f: 'food',
            b: 'beta',
            z: {
        ...
        let path;
        ({ path } = await FunctionUtils.tryAccumulatePath(exp, memory, undefined));
    ...
    it('Test Evaluation Options', async function () {
        const mockMemory = {};
        ...
        ({ value, error } = await exp.tryEvaluate(mockMemory, options));
