...
const { TypeFactory, TypeLoader } = require('../lib');
...
describe('TypeLoader', function () {
    ...
    it('TypeLoader TextPrompt: simple textprompt dialog should prompt for text', async function () {
        declarativeTestCase('00 - TextPrompt\\SimplePrompt.main.dialog', async function(adapter){
    ...
    it('TypeLoader Steps: should be ran when no other rules', async function () {
        declarativeTestCase('01 - Steps\\Steps.main.dialog', async function(adapter){
            await adapter.send('hi')
                .assertReply('Step 1')
                .assertReply('Step 2')
                .assertReply('Step 3');
    ...
    it('TypeLoader EndTurn: end turn should wait for user input', async function () {
        declarativeTestCase('02 - EndTurn\\EndTurn.main.dialog', async function(adapter){
            await adapter
            .send('hi')
    ...
    it('TypeLoader IntentRule: intent rule should route according to intent defined in recognizer', async function () {
        declarativeTestCase('06 - DoSteps\\DoSteps.main.dialog', async function(adapter){
            await adapter
            .send('hi')
                .assertReply('Hello, I\'m Zoidberg. What is your name?')
            .send('Carlos')
...
function declarativeTestCase(path, callback) {
    readPackageJson(`C:\\Users\\ccastro\\repos\\jstl\\botbuilder-js\\libraries\\botbuilder-dialogs-declarative\\tests\\declarative\\${path}`,
