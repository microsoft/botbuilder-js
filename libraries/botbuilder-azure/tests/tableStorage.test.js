...
testStorage = function () {
    ...
    */
    ...
    it('create large object', function () {
        ...
        return storage.write(changes)
            ...
            .then(result => {
                ...
                assert.equal(result.bigObject.text, bigString);
            })
            .catch(handleError);
    });
    ...
    it('update large object with etag', function () {
        var bigString = randomString(74000);
        var newString = randomString(100000);
        var changes = {
            bigObjectWithEtag: {
                text: bigString
            }
    ...
    it('update large object with smaller obj', function () {
        var bigString = randomString(74000);
        var newString = randomString(10);
        var changes = {
            bigToSmallObject: {
                text: bigString
            }
        ...
        return storage.write(changes)
            .then(() => storage.read(['bigToSmallObject']))
            .then((obj) => {
                obj.bigToSmallObject.text = newString;
                return storage.write(obj);
            })
            .then(() => storage.read(['bigToSmallObject']))
            .then(result => {
                assert(result != null, 'result should be object');
                assert(result.bigToSmallObject, 'bigToSmallObject is not defined');
                assert.equal(result.bigToSmallObject.text, newString);
            })
            .catch(handleError);
    });

    it('delete large object', function () {
        var bigString = randomString(74000);
        var changes = {
            objectToDelete: {
                text: bigString
        ...
        return storage.write(changes)
            .then(() => storage.delete(['objectToDelete']))
            .then(() => storage.read(['objectToDelete']))
            .then(result => {
                assert(result != null, 'result should be object');
                assert(!result.objectToDelete, 'objectToDelete should not be set');
            })
            .catch(handleError);
    })

    it('split big object into chunks', function () {
        var bigString = randomString(74000);
        var obj = { Text: bigString };

        var splitter = new StoreItemContainer("test", obj);
        var chunks = splitter.split();

        assert.equal(3, chunks.length);
    });

    it('join big object from chunks', function () {
        var bigString = randomString(74000);
        var obj = { Text: bigString };
        ...
        var joined = StoreItemContainer.join(chunks);

        assert.equal(bigString, joined.obj.Text);
    });

    it('join big object preserves original key', function () {
        var key = "!@#$%^&*()~/\\><,.?';\"`~";
        var bigString = randomString(74000);
        var obj = { Text: bigString };

        var splitter = new StoreItemContainer(key, obj);
        var chunks = splitter.split();
