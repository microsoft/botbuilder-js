...
describe('FileStorage', function () {
    let folder = path.join(os.tmpdir(), 'filestoragetest');
    before(function () {
        return fs.exists(folder)
            .then((exists) => (exists) ? fs.readdir(folder) : Promise.resolve())
            .then((files) => {
                if (files) {
                    let promises = [];
                    files.forEach(function (file, index) {
                        promises.push(fs.unlink(path.join(folder, file)));
                    });
    ...
    testStorage(new builder.FileStorage({ path: folder }));
});
