...
export class EventPlanStep extends PlanStep {
    ...
    public async recognizeStep(dc: DialogContext, recognized: RecognizerResult, planStack: PlanState[]): Promise<RecognizedPlanStep|undefined> {
        ...
        if (activity.type === ActivityTypes.Event && activity.name === this.name && await this.activationRule(dc, planStack)) {
            // Create a new recognizer result containing the events value
