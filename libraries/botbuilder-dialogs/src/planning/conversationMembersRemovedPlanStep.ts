...
export class ConversationMembersRemovedPlanStep extends PlanStep {
    public async recognizeStep(dc: DialogContext, recognized: RecognizerResult, planStack: PlanState[]): Promise<RecognizedPlanStep|undefined> {
        ...
        if (activity.type === ActivityTypes.ConversationUpdate && activity.membersRemoved && await this.activationRule(dc, planStack)) {
            // Find members
            const members: ChannelAccount[] = [];
            activity.membersRemoved.forEach((member) => {
