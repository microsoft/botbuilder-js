...
export class BotFrameworkAdapter extends BotAdapter implements IUserTokenProvider {
    ...
    public async processSkillRequest(req: WebRequest, res: WebResponse, logic: (context: TurnContext) => Promise<any>): Promise<void> {
        ...
        try {
            ...
            status = 404;
            const activity = await this.skillRequestToActivity(req);
    ...
    protected async skillRequestToActivity(req: WebRequest): Promise<Partial<Activity>> {
    ...
    public async forwardActivity(context: TurnContext, protocolUri: string, activity: Partial<Activity>): Promise<void> {
        // Decode protocol uri
        if (!protocolUri.startsWith('bf:')) { throw new Error(`BotFrameworkAdapter.forwardActivity: protocolUri not supported "${protocolUri}".`) }
        const options = {} as BFProtocol;
        protocolUri.substr(3).split('&').forEach((kv) => {
            const pair = kv.split('=');
        ...
        if (response.status >= 400) { throw new Error(`TurnContext: '${response.status}' error forwarding activity to '${protocolUri}'.`) }
    }
}

interface BFProtocol {
    url: string;
    appId: string;
}
