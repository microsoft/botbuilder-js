...
class TestAdapter {
    ...
    _sendActivityToBot(userSays) {
        ...
        let activity = (typeof userSays === 'string' ? { type: botframework_connector_1.ActivityTypes.message, text: userSays } : userSays);
        if (!activity.type)
            ...
            return new Promise((resolve, reject) => {
                ...
                let myInterval = setInterval(() => {
                    ...
                    if ((current - start) > timeout) {
                        let expectedActivity = (typeof expected === 'string' ? { type: botframework_connector_1.ActivityTypes.message, text: expected } : expected);
                    ...
                    if (this.adapter.botReplies.length > 0) {
                        ...
                        if (typeof expected === 'function') {
                            ...
                            assert.equal(botReply.type, botframework_connector_1.ActivityTypes.message, (description || '') + ` type === '${botReply.type}'. `);
