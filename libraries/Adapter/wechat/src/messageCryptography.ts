...
function AesDecrypt(encryptString: string, encodingAesKey: string, appId: string): string {
    ...
    var decipherBuff = Buffer.concat([decipher.update(encryptString, 'base64'), decipher.final()]);
    decipherBuff = PKCS7Decoder(decipherBuff);
    var len = decipherBuff.slice(16);
    var msgLen = len.slice(0, 4).readUInt32BE(0);
    var result = len.slice(4, msgLen + 4).toString();
    var appid = len.slice(msgLen + 4).toString();
    if (appId !== appid) {
