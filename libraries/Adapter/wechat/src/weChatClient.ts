...
export class WeChatClient {
    ...
    public async UploadNewsAsync(newsList: News[], isTemporary: boolean, timeout = 30000): Promise<UploadMediaResult> {
        ...
        if (cacheResult === undefined || cacheResult.Expired) {
            ...
            const result = await this.SendHttpRequestAsync('POST', url, data, null, timeout);
    ...
    public async SendHttpRequestAsync(method: HttpMethods, url: string, data: any = undefined, token: string = undefined, timeout = 10000): Promise<any> {
        const result = await this.MakeHttpRequestAsync(method, url, data, token, timeout);
        return result;
    }
     ...
     */
    private async MakeHttpRequestAsync(method: HttpMethods, url: string, data: any = undefined, token: string = undefined, timeout = 10000): Promise<any> {
        ...
        const response = await this.Client.sendRequest(requestMessage);
     ...
     */
    private async ProcessUploadMediaAsync(attachmentData: AttachmentData, url: string, isTemporaryMeida: boolean, timeout = 30000): Promise<UploadMediaResult> {
        try {
            ...
            const response = await this.SendHttpRequestAsync('POST', url, form, null, timeout);
            if (isTemporaryMeida) {
