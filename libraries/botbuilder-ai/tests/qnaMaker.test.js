...
describe('QnAMaker', function () {
    ...
    beforeEach(function () {
        ...
        if (mockQnA) {
            ...
            const arr = testFiles.filter((file) => file.startsWith(fileName + '.'));
    ...
    describe('Instantiation', () => {
        it('should instantiate a QnAMaker class successfully with QnAMakerEndpoint, but without options specified', () => {
            new QnAMaker(endpoint);
        });

        it('should instantiate a QnAMaker class successfully with both QnAMakerEndpoint and QnAMakerOptions args', () => {
        ...
        it('should throw an error instantiating without QnAMakerEndpoint', () => {
            assert.throws(() => new QnAMaker(), new TypeError('QnAMaker requires valid QnAMakerEndpoint.'));
        });

        it('should throw TypeError if QnAMakerOptions.scoreThreshold is not a number during QnAMaker instantiation', () => {
            ...
            const nonNumberError = new TypeError(
                `"${stringScoreThreshold_options.scoreThreshold}" is an invalid scoreThreshold. QnAMakerOptions.scoreThreshold must have a value between 0 and 1.`
            );
            assert.throws(() => new QnAMaker(context, stringScoreThreshold_options), nonNumberError);
        });

        it('should throw error if QnAMakerOptions.scoreThreshold is not a number between 0 and 1 during QnAMaker instantiation.', () => {
            ...
            const error = new TypeError(
                `"${outOfRangeScoreThreshold_options.scoreThreshold}" is an invalid scoreThreshold. QnAMakerOptions.scoreThreshold must have a value between 0 and 1.`
            );
            assert.throws(() => new QnAMaker(context, outOfRangeScoreThreshold_options), error);
        });

        it('should not throw error if QnAMakerOptions.scoreThreshold is 1.', () => {
            const context = new TestContext({ text: 'do woodpeckers get concussions?' });
            const options = { scoreThreshold: 1 };
            assert.doesNotThrow(() => new QnAMaker(context, options));
        });

        it('should throw RangeError if QnAMakerOptions.top is not an integer during instantiation', () => {
            const context = new TestContext({ text: 'what if ostriches could fly?' });
            const decimalNumberTopOptions = { top: 2.5 };
            const notAnIntegerError = new RangeError(
                `"${decimalNumberTopOptions.top}" is an invalid top value. QnAMakerOptions.top must be an integer greater than 0.`
            );
            assert.throws(() => new QnAMaker(context, decimalNumberTopOptions), notAnIntegerError);
        });

        it('should throw error if QnAMaker.top is not a number during instantiation', () => {
            const context = new TestContext({ text: 'why were there sloths at the post office?' });
            const objectTopOptions = { top: { key: 'value' } };
            const nonNumberError = new RangeError(
                `"${objectTopOptions.top}" is an invalid top value. QnAMakerOptions.top must be an integer greater than 0.`
            );
            assert.throws(() => new QnAMaker(context, objectTopOptions), nonNumberError);
        });

        it('should throw RangeError if QnAMaker.top is not an integer greater than 1', () => {
            const context = new TestContext({ text: 'why did my son think the wasabi was guacamole?' });
            const smallerThanOneTopOptions = { top: -1 };
            const notGreaterThanOneError = new RangeError(
                `"${smallerThanOneTopOptions.top}" is an invalid top value. QnAMakerOptions.top must be an integer greater than 0.`
            );
            assert.throws(() => new QnAMaker(context, smallerThanOneTopOptions), notGreaterThanOneError);
        });

        it('null telemetry should work', () => {
            const options = { top: 7 };
        ...
        it('null telemetry logPersonalInformation should work', () => {
            const options = { top: 7 };
    ...
    describe('getAnswers()', () => {
        it('should return answer without any options specified', async () => {
            ...
            assert.strictEqual(
                ...
                'Should return only 1 answer with default settings (i.e. no options specified) for question with answer.'
            );
        });

        it('should return answer and active learning flag', async () => {
            const noOptionsQnA = new QnAMaker(endpoint);
            const noOptionsContext = new TestContext({ text: 'where are the unicorns?' });
            const defaultNumberOfAnswers = 1;
        ...
        it('should sort the answers in the qna results from highest to lowest score', async () => {
            ...
            assert.strictEqual(
                qnaResults,
                descendingQnaResults,
                'answers should be sorted from greatest to least score'
            );
        });

        it('should return answer with prompts', async () => {
            const qna = new QnAMaker(endpoint);
        ...
        it('should return answer with high score provided context', async () => {
            const qna = new QnAMaker(endpoint);
        ...
        it('should return answer with high score provided id', async () => {
            const qna = new QnAMaker(endpoint);
            const turnContext = new TestContext({ text: 'where can I buy?' });
        ...
        it('should return answer with low score not provided context', async () => {
            const qna = new QnAMaker(endpoint);
            const turnContext = new TestContext({ text: 'where can I buy?' });
        ...
        it('should call qnamaker with isTest true', async () => {
            const qna = new QnAMaker(endpoint);
        ...
        it('should call qnamaker with rankerType questionOnly', async () => {
            const qna = new QnAMaker(endpoint);
            const turnContext = new TestContext({ text: 'Q11' });
        ...
        it('should return answer with timeout option specified', async () => {
        ...
        it('should convert legacy response property "qnaId" to "id"', async () => {
            ...
            assert.strictEqual(
                hasIdInResponse(formattedQnaResult),
                hasIdInResponse(expectedQnaResultFormat),
                'Legacy QnA responses should have property "qnaId" converted to "id".'
            );
        });

        it('should return 0 answers for an empty or undefined utterance', async () => {
            ...
            assert.notStrictEqual(
                ...
                true,
                'Empty utterance should return an empty array, short circuiting call to QnA service.'
            );
            assert.strictEqual(
                emptyQuestionResults.length,
                0,
                'Should have not received answers for an empty utterance.'
            );

            const undefinedQuestionResults = await qna.getAnswers(undefinedUtteranceContext);
            assert.strictEqual(
                undefinedQuestionResults.length,
                0,
                'Should have not received answers for an undefined utterance.'
            );
        });

        it('should return 0 answers for questions without an answer.', async () => {
            const qna = new QnAMaker(endpoint, { top: 1 });
            ...
            assert.strictEqual(
                resultsWithoutAns.length,
                0,
                `Should have not received answers for a question with no answer, it returned ${JSON.stringify(
                    resultsWithoutAns
                )}.`
            );
        });

        it('should throw TypeError if QnAMakerOptions.scoreThreshold is not a number in service call', () => {
        ...
        it('should filter low score variation', async () => {
        ...
        it('should log telemetry when no answer found in kb', async () => {
            // Arrange
            var callCount = 0;
            var telemetryClient = {
                trackEvent: (telemetry) => {
            ...
            assert.strictEqual(
                numberOfResults,
                defaultNumberOfAnswers,
                'Results should have 0 answers if no match found in KB'
            );
        });

        it('should not log telemetry pii', async () => {
            // Arrange
            var callCount = 0;
            var telemetryClient = {
                trackEvent: (telemetry) => {
        ...
        it('should log telemetry using derived qna', async () => {
            // Arrange
            var callCount = 0;
            var telemetryClient = {
                trackEvent: (telemetry) => {
        ...
        it('should log telemetry additionalprops', async () => {
            // Arrange
            var callCount = 0;
            var telemetryClient = {
                trackEvent: (telemetry) => {
            ...
            // Act
            const resultsWithoutOptions = await noOptionsQnA.getAnswers(
                noOptionsContext,
                null,
    ...
    describe('emitTraceInfo()', () => {
        it('method should throw error if context is undefined', async () => {
            const qna = new QnAMaker(endpoint);

            assert.rejects(
                async () => await qna.getAnswers(undefined),
                new TypeError('QnAMaker.getAnswers() requires a TurnContext.')
            );
        });
    });

    describe('trainAPI()', async () => {
        it('should call train async function', async () => {
            const qna = new QnAMaker(endpoint);
    ...
    describe('Deprecated QnA Methods', () => {
        it('should work free standing', async () => {
            const qna = new QnAMaker(endpoint, { top: 1 });
            ...
            assert.strictEqual(
                firstCallQnaResults.length,
                1,
                'Should have received just one answer on the first call.'
            );
            assert.strictEqual(
                firstCallQnaResults[0].answer.startsWith(answer),
                true,
                `The answer should have started with '${answer}' for the first call.`
            );
            ...
            assert.strictEqual(
                secondCallQnaResults.length,
                1,
                'Should have received just one answer on the second call.'
            );
            assert.strictEqual(
                secondCallQnaResults[0].answer.startsWith(answer),
                true,
                `The answer should have started with '${answer}' for the second call.`
            );
        });

        it('should return 0 answers for a question with no answer after a succesful call', async () => {
            const qna = new QnAMaker(endpoint, { top: 1 });
            ...
            assert.strictEqual(
                resultsWithAns[0].answer.startsWith(answer),
                true,
                `The answer should have started with '${answer}' for the first call.`
            );
            ...
            assert.strictEqual(
                resultsWithoutAns.length,
                0,
                'Should have not received answers for a question with no answers.'
            );
        });
        ...
        it('should return 0 answers for questions without an answer.', async () => {
            ...
            assert.strictEqual(
                undefinedQuestionResults.length,
                0,
                'Should have not received answers for an undefined question.'
            );
        });

        it('should emit trace info once per call to Answer', async () => {
            ...
            assert.strictEqual(
                traceActivity.hasOwnProperty('value'),
                true,
                `'traceActivity' should have 'value' property.`
            );
            assert.strictEqual(
                traceActivity.value.hasOwnProperty('message'),
                true,
                `'traceActivity.value' should have 'message' property.`
            );
            assert.strictEqual(
                traceActivity.value.hasOwnProperty('queryResults'),
                true,
            ...
            assert.strictEqual(
                traceActivity.value.hasOwnProperty('scoreThreshold'),
                true,
                `'traceActivity.value' should have 'scoreThreshold' property.'`
            );
        });

        it('should return "false" from answer() if no good answers found', async () => {
        ...
        it('should throw TypeError from answer() if no context', async () => {
            const qna = new QnAMaker(endpoint);
            const context = undefined;
            const contextMissingError = new TypeError('QnAMaker.answer() requires a TurnContext.');
            await assert.rejects(async () => await qna.answer(context), contextMissingError);
        });

        it('should sort results from generateAnswers in descending order', async () => {
            const qna = new QnAMaker(endpoint);
        ...
        it('should_return_an_answer_for_question_with_answer_with_QnAMakerOptions.strictFilters_applied', async () => {
            const qna = new QnAMaker(endpoint);
            ...
            const qnaResults = await qna.generateAnswer(question, filters);
            assert.strictEqual(qnaResults.length, 1, 'answer is returned for strictFilter OR');
        });

        it('should_return_an_answer_for_question_with_answer_with_QnAMakerOptions.strictFiltersoperation_applied', async () => {
            const qna = new QnAMaker(endpoint);
            const question = 'what happens when you hug a procupine?';
    ...
    describe('Active Learning', () => {
        ...
        it('callTrainAsync() should send correct payload body to Train API', async () => {
            nock(endpoint.host).post(trainApi).reply(204);
            const trainUtils = new TrainUtils(endpoint);
            await assert.doesNotReject(() => trainUtils.callTrain(feedbackRecords));
        });
    });
});
