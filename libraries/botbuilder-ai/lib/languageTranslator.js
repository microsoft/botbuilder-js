...
class PostProcessTranslator {
    ...
    splitSentence(sentence, alignments, isSrcSentence = true) {
        ...
        if (isSrcSentence) {
            ...
            alignments.sort((a, b) => {
                let aIndex = parseInt(a.split('-')[wrdIndexInAlignment].split(':')[0], 10);
                let bIndex = parseInt(b.split('-')[wrdIndexInAlignment].split(':')[0], 10);
                if (aIndex <= bIndex) {
                    return -1;
                }
                else {
        ...
        alignments.forEach(alignData => {
            ...
            let startIndex = parseInt(wordIndexes.split(':')[0], 10);
            let length = parseInt(wordIndexes.split(':')[1], 10) - startIndex + 1;
    ...
    wordAlignmentParse(alignments, srcWords, trgWords) {
        ...
        alignments.forEach(alignData => {
            ...
            let srcStartIndex = parseInt(wordIndexes[0].split(':')[0], 10);
            let srcLength = parseInt(wordIndexes[0].split(':')[1], 10) - srcStartIndex + 1;
            let srcWrd = sourceMessage.substr(srcStartIndex, srcLength);
            let srcWrdIndex = srcWords.findIndex(wrd => wrd === srcWrd);
            let trgstartIndex = parseInt(wordIndexes[1].split(':')[0], 10);
            let trgLength = parseInt(wordIndexes[1].split(':')[1], 10) - trgstartIndex + 1;
