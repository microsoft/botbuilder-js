...
export const codesToNames: { [key: string]: string } = {};
export const namesToCode: { [key: string]: string } = {};
export const namesToNames: { [key: string]: string[] } = {};
...
namesToNames["Chinese"] = ["Simplified", "Traditional"];
...
namesToCode["querétaro"] = "otq";
namesToCode["otomi"] = "otq";
...
namesToNames["Serbian"] = ["Cyrillic", "Latin"];
...
namesToCode["yucatec"] = "yua";
namesToCode["maya"] = "yua";

function AddLanguageMap(code: string, name: string): void {
    codesToNames[code] = name.toLowerCase();
    namesToCode[name.toLowerCase()] = code;
}

function GetCodeForInput(values: string[]): string {
    let code: string;
    for (let iValue in values) {
        let value = values[iValue];

        if (namesToCode[value])
            return namesToCode[value];
...
function GetCodeOrFallback(code: string): string {
