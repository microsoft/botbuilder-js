 ...
 * @param {PropertyStateAccessor} lgEntitiesState property accessor for user state
 */
class Greeting extends ComponentDialog {
    constructor(dialogId, userProfileAccessor, lgEntitiesState) {
        super(dialogId);
    ...
    async promptForNameStep(step) {
        ...
        if (!userProfile.name) {
            ...
            return await step.prompt(NAME_PROMPT, templateReferences.nameQuestion);
        } else {
            return await step.next();
        }
    }
