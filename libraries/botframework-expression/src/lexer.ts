...
export abstract class Lexer {
    ...
    public static readonly Patterns: string[] = ["[+-]?\\d+(?:\.\\d+)?", "'[^']*'", "[a-z][a-z0-9]*"]
        ...
        .select((e) => e.Token).distinct().orderByDescending((e) => e.length)
        .select((e) => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).toArray());
    ...
    public static *Tokens(text: string): IterableIterator<Token> {
        while (text !== undefined && text != null && text.length !== 0) {
            const groups = Lexer.Matcher.exec(text);
