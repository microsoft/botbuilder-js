 ...
 */
export const noRetry: RetryPolicy = () => Promise.resolve(false);
 ...
 * @returns always true
 */
export const retryAny: ErrorFilter = () => true;
 ...
 *
 * @returns Unconditionally returns a 0 milliseconds delay
 */
...
export function exponentialRetry(initialDelay: number, maxDelay: number): RetryDelay {
    const maxExponentiationAttempts = maxDelay > initialDelay ? Math.floor(Math.log2(maxDelay / initialDelay)) : 0;
    ...
    return (attempt) => {
        return attempt < maxExponentiationAttempts ? initialDelay * 2 ** attempt : maxDelay;
    };
}

/**
 * Gets a retry policy which will retry according to the supplied settings.
 *
 * @param settings The settings used to configure the retry policy
 * @returns The retry policy
 */
export function retryPolicy(settings: RetrySettings): RetryPolicy {
    const { maxAttempts, retryDelay, errorFilter = retryAny } = settings;
...
function delay(interval: number): Promise<void> {
    return interval > 0 ? new Promise((resolve) => setTimeout(resolve, interval)) : Promise.resolve();
}
