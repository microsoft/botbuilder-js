...
export abstract class AbstractDialogFlowTask<R, O = Jsonify<R>> implements DialogFlowTask<R, O> {
     ...
     */
    configureRetry(retryConfig: boolean | RetrySettings | RetryPolicy): this {
     ...
     * @inheritdoc
     */
    abstract then<T>(continuation: (value: R, context: TurnContext) => T | Promise<T>): DialogFlowTask<T>;
     ...
     * @param projectorOrSchema The zod schema used to parse the result or the projector function
     * @returns A new task instance.
     */
    project<T>(projectorOrSchema: ZodType<T> | ((value: Jsonify<R>) => T)): DialogFlowTask<R, T> {
        const projector =
            projectorOrSchema instanceof ZodType ? (value) => projectorOrSchema.parse(value) : projectorOrSchema;
    ...
    *result(): Generator<DialogFlowTask, O, O> {
    ...
    protected async applyRetryPolicy<T extends (...args: any[]) => Promise<R>>(
        ...
        for (let attempt: number = 1; ; ++attempt) {
            const result = await this.invokeTask(task, ...args);
