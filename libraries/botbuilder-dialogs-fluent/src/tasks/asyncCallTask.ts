...
export class AsyncCallTask<R, O = Jsonify<R>> extends AbstractDialogFlowTask<R, O> {
     ...
     * @inheritdoc
     */
    override then<T>(continuation: (value: R, context: TurnContext) => T | Promise<T>): DialogFlowTask<T> {
        return Object.assign(this.project(defaultProjector) as AsyncCallTask<R>, {
     ...
     */
    invoke(context: TurnContext): Promise<TaskResult<R>> {
