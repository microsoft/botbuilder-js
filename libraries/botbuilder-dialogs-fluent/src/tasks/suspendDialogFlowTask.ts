...
export abstract class SuspendDialogFlowTask<R, O = Jsonify<R>> extends AbstractDialogFlowTask<R, O> {
     ...
     * @inheritdoc
     */
    override then<T>(continuation: (value: R, context: TurnContext) => T | Promise<T>): DialogFlowTask<T, Jsonify<T>> {
        return Object.assign(this.project(defaultProjector) as SuspendDialogFlowTask<R>, {
            resume: (context, result) => this.resume(context, result).then((prev) => continuation(prev, context)),
        });
    }
     ...
     */
    abstract onSuspend(dialogContext: DialogContext): Promise<DialogTurnResult>;
     ...
     */
    onResume(turnContext: TurnContext, result: any): Promise<TaskResult<R>> {
