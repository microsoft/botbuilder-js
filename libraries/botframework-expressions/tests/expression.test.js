...
const dataSource = [
    ...
    ['join(foreach(doubleNestedItems, newitems, join(foreach(newitems, item, item.x), ",")), ",")', '1,2,3'],
    ['join(foreach(doubleNestedItems, newitems, join(foreach(newitems, item, concat(y, string(item.x))), ",")), ",")', 'y1,y2,y3'],
    ['count(where(doubleNestedItems, newitems, count(where(newitems, item, item.x == 1)) == 1))', 1],
    ['count(where(doubleNestedItems, newitems, count(where(newitems, item, count(newitems) == 1)) == 1))', 1],
