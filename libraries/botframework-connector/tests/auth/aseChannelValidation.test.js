const { AseChannelValidation, GovernmentConstants, AuthenticationConstants, BetweenBotAndAseChannelTokenValidationParameters } = require('../..');
...
describe('AseChannelTestSuite', function () {
    describe('AseChannelTestCase', function () {
        ...
        it('ValidationMetadataUrlTest_AseChannel_USGov', function () {
            ...
            assert.strictEqual(AseChannelValidation.MetadataUrl, GovernmentConstants.ToBotFromEmulatorOpenIdMetadataUrl);
        ...
        it('ValidationIssueUrlTest_AseChannel', function () {
            ...
            tenantIds.forEach(function (tmpId) {
                assert.strictEqual(true, BetweenBotAndAseChannelTokenValidationParameters.issuer.includes(`https://sts.windows.net/${tmpId}/`));
                assert.strictEqual(true, BetweenBotAndAseChannelTokenValidationParameters.issuer.includes(`https://login.microsoftonline.com/${tmpId}/v2.0`));
                assert.strictEqual(true, BetweenBotAndAseChannelTokenValidationParameters.issuer.includes(`https://login.microsoftonline.us/${tmpId}/v2.0`));
            })
        });
