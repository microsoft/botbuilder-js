    ...
    describe('Conversations', function() {
        ...
        describe('GetConversationMembers', function() {
            ...
            it('should fail with invalid conversationId', async function () {
                ...
                const result = await client.conversations.createConversation(params);
                await assert.rejects(async () => await client.conversations.getConversationMembers(result.id.concat('M')),
                {
                    code:'BadArgument',
        ...
        describe('SendToConversation', function() {
            ...
            it('should fail with invalid conversationId', async function () {
                ...
                const result = await client.conversations.createConversation(params);
                await assert.rejects(async () => await client.conversations.sendToConversation(result.id.concat('M'), createActivity()),
                {
                    code: 'BadArgument',
                    message: 'Slack API error'
                });
        ...
        describe('GetActivityMembers', function() {
            ...
            it('should fail with invalid conversationId', async function () {
                ...
                const result = await client.conversations.createConversation(params);
                await assert.rejects(async () => await client.conversations.getActivityMembers(result.id.concat('M'), result.activityId),
                {
                    code: 'BadArgument',
                    message: 'Slack API error',
                });
        ...
        describe('ReplyToActivity', function() {
            ...
            it('should fail with invalid conversationId', async function () {
                ...
                const result2 = await client.conversations.sendToConversation(result.id, createActivity());
                await assert.rejects(async () => await client.conversations.replyToActivity('invalid-id', result2.id, createActivity()),
                {
                    code: 'ServiceError',
        ...
        describe('DeleteActivity', function() {
            it('should delete the activity', async function () {
                ...
                await assert.doesNotReject(async () => await client.conversations.deleteActivity(result.id, result.activityId));
            });
            it('should fail with invalid conversationId', async function () {
                const result = await client.conversations.createConversation(createConversation());
                const result2 = await client.conversations.sendToConversation(result.id, createActivity());
        ...
        describe('UpdateActivity', function() {
            ...
            it('should fail with invalid conversationId', async function () {
                const result = await client.conversations.createConversation(createConversation());
                const result2 = await client.conversations.sendToConversation(result.id, createActivity());
                await assert.rejects(async () => client.conversations.updateActivity('invalid-id', result2.id, createActivity()),
                {
                    code: 'ServiceError',
                    message: 'Invalid ConversationId: invalid-id',
                });
    ...
    describe('TokenApiClient', function() {
        describe('tokenApiClient Construction', function() {
            ...
            it('should throw on null credentials', function () {
                assert.throws(() => new TokenApiClient(null, {
                    baseUri: 'http://localhost'
                }),
                {
        ...
        describe('userToken', function() {
            ...
            describe('getAadTokens', function() {
                it('should throw on null userId', async function () {
                    await assert.rejects(async () => await tokenApiClient.userToken.getAadTokens(null, 'mockConnection', { resourceUrls: ['http://localhost' ]}),
                    {
                        message: 'userId cannot be null'
                    });
                });
                it('should throw on null connectionName', async function () {
                    await assert.rejects(async () => await tokenApiClient.userToken.getAadTokens(user.id, null, { resourceUrls: ['http://localhost' ]}),
                    {
                        message: 'connectionName cannot be null'
                    });
                });
                it('should return token', async function () {
                    const result = await tokenApiClient.userToken.getAadTokens(user.id, 'slack', { resourceUrls: ['http://localhost' ]});
                    assert(result.channelId);
                    assert(result.connectionName);
                    assert(result.token);
                    assert(result.expiration);
