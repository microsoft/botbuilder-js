...
import { AuthSettings, BotAuthenticatorSettings, BotCredentials } from './settings';
...
export class BotAuthenticator {
    ...
    private settings: BotAuthenticatorSettings;

    constructor(protected credentials: BotCredentials = {}) {
        this.settings = <BotAuthenticatorSettings>credentials;
    ...
    public authenticate(headers: Headers, channelId?: string, serviceUrl?: string): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            ...
            if (!!token) {
                ...
                var verifyOptions: jwt.VerifyOptions | null = null;
                var openIdMetadata: OpenIdMetadata | null = null;
                ...
                if (openIdMetadata) {
                    openIdMetadata.getKey(decoded.header.kid, key => {
                        if (key) {
                            try {
                                jwt.verify(token, key.key, <jwt.VerifyOptions>verifyOptions);
