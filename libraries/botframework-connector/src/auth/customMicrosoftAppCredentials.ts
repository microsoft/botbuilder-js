 ...
 */
export class CustomMicrosoftAppCredentials {
    ...
    public static trustServiceUrl(serviceUrl: string, expiration?: Date): void {
        ...
        if (uri.host) {
            CustomMicrosoftAppCredentials.trustedHostNames.set(uri.host, expiration);
        }
    }
    ...
    public static isTrustedServiceUrl(serviceUrl: string): boolean {
        try {
            const uri: url.Url = url.parse(serviceUrl);
            if (uri.host) {
                return CustomMicrosoftAppCredentials.isTrustedUrl(uri.host);
            }
        } catch (e) {
    ...
    private static isTrustedUrl(uri: string): boolean {
        const expiration: Date = CustomMicrosoftAppCredentials.trustedHostNames.get(uri);
        if (expiration) {
    ...
    public async getToken(forceRefresh: boolean = false): Promise<string> {
        if (!forceRefresh) {
            ...
            const oAuthToken: OAuthResponse = CustomMicrosoftAppCredentials.cache.get(this.tokenCacheKey);
            if (oAuthToken) {
        ...
        if (res.ok) {          
            ...
            if (!res.bodyUsed){
                ...
                CustomMicrosoftAppCredentials.cache.set(this.tokenCacheKey, oauthResponse);
