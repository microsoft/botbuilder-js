...
class RootDialog extends botbuilder_rules_1.AdaptiveDialog {
    constructor() {
        super('main');
        // Bind to production/development recognizer
        this.recognizer = recognizer_1.getRecognizer();
        // Define welcome rule
        this.addRule(new botbuilder_rules_1.WelcomeRule([
        ...
        this.addRule(new botbuilder_rules_1.IntentRule(schema_1.intents.AddToDo, [
            new addToDo_1.AddToDo()
        ]));
        this.addRule(new botbuilder_rules_1.IntentRule(schema_1.intents.DeleteToDo, [
            new deleteToDo_1.DeleteToDo()
        ]));
        this.addRule(new botbuilder_rules_1.IntentRule(schema_1.intents.ClearToDos, [
            new clearToDos_1.ClearToDos()
        ]));
        this.addRule(new botbuilder_rules_1.IntentRule(schema_1.intents.ShowToDos, [
            new showToDos_1.ShowToDos()
        ], botbuilder_rules_1.PlanChangeType.doSteps));
        // Define rules to handle cancel events
        this.addRule(new botbuilder_rules_1.EventRule(schema_1.events.CancelAdd, [
            new botbuilder_rules_1.SendActivity(`Ok... Cancelled adding new alarm.`)
        ]));
        this.addRule(new botbuilder_rules_1.EventRule(schema_1.events.CancelDelete, [
            new botbuilder_rules_1.SendActivity(`Ok...`)
        ]));
        // Define rules for handling errors
        this.addRule(new botbuilder_rules_1.EventRule(schema_1.events.Error, [
            new botbuilder_rules_1.SendActivity(`Oops. An error occurred: {message}`)
        ]));
        // Define rule for default response
        this.addRule(new botbuilder_rules_1.DefaultRule([
